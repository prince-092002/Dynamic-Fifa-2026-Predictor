...
- **Streamlit Cloud: DEPLOYED, but set to PRIVATE.** App live at **https://dynamic-fifa-2026-predictor-aacmn59qg3ueeusybvx9xw.streamlit.app** (`dashboard/app.py`, branch `main`). It currently returns HTTP 303 → `share.streamlit.io/-/auth/app` (viewer login required). **User must flip it to Public** in Streamlit app Settings → Sharing (turn off "require viewers to sign in"). URL is wired into `website/.env.production` (`NEXT_PUBLIC_DASHBOARD_URL`) and README; the website's Dashboard button is live (verified). No secrets on Streamlit.
